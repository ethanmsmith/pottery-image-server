create table if not exists user(guid text, githublogin text, name text, avatar text, github_token text, PRIMARY KEY (guid));
create table if not exists photos(guid text, created text, description text, name text, postedby text CONSTRAINT fk_user REFERENCES user (guid), category textPRIMARY KEY (guid));
